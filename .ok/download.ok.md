...
- The public download module surface is checked in at `src/download/lib.zig`, which currently re-exports the database interface module, `commands.random`, `RandomPlaybackLoop`, `ScreensaverPlaybackLoop`, `protocols.http`, and `storage.paths` plus `storage.files`.
...
- The checked-in Stage 1 screensaver runtime is evidenced in `src/download/commands/random.zig` and `src/download/commands/random_test.zig`: `ScreensaverPlaybackLoop` wraps `RandomPlaybackLoop`, enters and restores the alternate screen, hides and restores the cursor, polls stdin for exit, and installs best-effort SIGINT/SIGTERM cleanup hooks.
...
- Current archive-source reality is checked in at `src/download/database/interface.zig` and `src/download/database/hardcoded.zig`: `ArchiveDatabase.init` always selects the hardcoded implementation, `getPack` returns `error.NotImplemented`, `searchFiles("CXC-STICK")` returns the curated `mist1025/CXC-STICK.ASC` file metadata, `listPacksByYear(2025)` returns the curated `mist1025` pack metadata, and the curated file list still contains only that single remote artwork entry.
...
- Current first-use art-supply evidence is still negative for looping playback: the repo does not yet ship starter-art provisioning, bootstrap, or a package-owned first-run seeding path that materializes playable artwork into `random/`, `packs/`, or `local/` before `random` or `screensaver` starts.
...
- Evidence for SQLite indexing, pack extraction, pack browsing, mirror sync, alias executables, package-owned starter-pool provisioning, or protocol fallback is currently negative: no checked-in code, tests, or stable artifacts promote those behaviors beyond spec intent.
...
- TRUE: The public `download` module surface currently includes both `RandomPlaybackLoop` and `ScreensaverPlaybackLoop` as direct exports from `src/download/lib.zig`; `ScreensaverPlaybackLoop` is Stage 1 runtime truth, not just an internal implementation detail.
...
- TRUE: Current archive database behavior is slightly broader than a pure random-only stub: the hardcoded implementation now returns narrow curated results for `searchFiles("CXC-STICK")` and `listPacksByYear(2025)`, while `getPack` remains demoted and returns `error.NotImplemented`.
- TRUE: Current first-use art supply for looping playback is still manual or indirect: `random` and `screensaver` require a non-empty local pool, and the repo does not yet evidence starter-art provisioning, explicit bootstrap, or a package-owned first-run seeding path.
...
- TRUE: Current Stage 1 screensaver behavior is terminal-session aware: it switches to the alternate screen only when stdout is a TTY, hides/restores the cursor, exits on stdin activity, and restores terminal state on SIGINT/SIGTERM according to the checked-in runtime and tests.
...
- FALSE: The current single-file remote fallback or manual `random-1` seeding path is mistaken for a supported zero-setup endless-screensaver solution.
- FALSE: First-use `random` or `screensaver` startup is defined around `.index.db`, search, pack download, or mirror sync instead of a smaller already-local starter-pool guarantee.
- FALSE: Starter-pool provisioning, explicit bootstrap, and later archive growth are collapsed into one undifferentiated "download client" claim.
...
- FALSE: `ScreensaverPlaybackLoop` is omitted from public-surface reconciliation or treated as a private helper when `src/download/lib.zig` exports it directly and Stage 1 tests pin its terminal-session behavior.
...
- TRUE: Any future claim that `random` or `screensaver` works on a fresh machine must distinguish starter-pool provisioning from optional bootstrap, local archive growth, and later `.index.db` authority.
- TRUE: Any future `16c` package or installer path that claims immediate looping playback must cite both the repo-owned launch/package artifact and the code/tests that materialize starter art into the current local playable pool.
